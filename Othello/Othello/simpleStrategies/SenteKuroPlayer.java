...
public class SenteKuroPlayer extends Strategy {
	...
	public Move nextMove(GameState currentState, int remainingTime) {
		...
		*/
		...
		System.out.println("move: x="+ m.x + ", y=" + m.y );
		this.count++;
	...
	private Move commonStrategy(GameState currentState){
		...
		if(check_state[0][0]==3){ //角
			...
			yoko=5;
			tate=7;
		}else if(check_state[0][1]==3 && check_state[0][0]==1){
			yoko=0;
			tate=1;
		}else if(check_state[1][0]==3 && check_state[0][0]==1){
			yoko=1;
			tate=0;
		}else if(check_state[6][0]==3 && check_state[7][0]==1){
			yoko=6;
			tate=0;
		}else if(check_state[7][1]==3 && check_state[7][0]==1){
			yoko=7;
			tate=1;
		}else if(check_state[0][6]==3 && check_state[0][7]==1){
			yoko=0;
			tate=6;
		}else if(check_state[1][7]==3 && check_state[0][7]==1){
			yoko=1;
			tate=7;
		}else if(check_state[6][7]==3 && check_state[7][7]==1){
			yoko=6;
			tate=7;
		}else if(check_state[7][6]==3 && check_state[7][7]==1){
			yoko=7;
			tate=6;
		}else {
			for(int i=5;i>1;i--){
				if(check_state[i][0]==3){
					if(check_state[i-1][0]==1 || check_state[i+1][0]==1){
						move.x=i;
						move.y=0;
						return move;
					}
				}else if(check_state[i][7]==3){
					if(check_state[i-1][7]==1 || check_state[i+1][7]==1){
						move.x=i;
						move.y=7;
						return move;
					}
				}else if(check_state[0][i]==3){
					if(check_state[0][i-1]==1 || check_state[0][i+1]==1){
						move.x=0;
						move.y=i;
						return move;
					}
				}else if(check_state[7][i]==3){
					if(check_state[7][i-1]==1 || check_state[7][i+1]==1){
						move.x=7;
						move.y=i;
						return move;
					}
